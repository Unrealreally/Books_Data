...
df.to_sql('books', connection, if_exists='replace', index=False)  # Save the DataFrame to the SQLite database


query_result = connection.cursor().execute("SELECT * FROM books ORDER BY price DESC LIMIT 5").fetchall()
top5books = pd.read_sql("SELECT * FROM books ORDER BY price DESC LIMIT 5", connection)
